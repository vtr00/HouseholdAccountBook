...
It is supported for Japanese and English Language, and for multi assets.

## __概要__
本ソフトは複数のアセット(通貨、証券、暗号資産、ポイントなど)に対応した家計簿ソフトです。  
データベースとして、SQLiteおよびPostgreSQLに対応しています。また、それらの相互乗り換えも可能です。  
DBアクセスを高速に実行したいのであれば、PostgreSQLの使用をお勧めしますが、環境の構築に手間が掛かるため、手軽に利用したい場合はSQLiteをお勧めします。  
...
* その他の機能
	...
	* ツール関連
		* 「店舗」「備考」の選択候補からの削除
		* 所属する帳簿項目が存在するときに、「アセット」「帳簿」「分類」「項目」及び帳簿と項目の関連設定の削除抑止
		* csvファイル
			* 照合
			* 追加/編集
	* 操作関連
...
## __SQLiteでの使用方法__
1. 本ソフトを起動し、「ツール」>「設定」で、必要なマスタデータの作成を行います。
    - アセット、帳簿、分類、項目、帳簿と項目の関連付け
1. さあ、記帳しましょう。
...
1. 本ソフトを起動し、「ツール」>「設定」>「DB設定」>「データベース通信設定を再登録する(再起動)」を選択し、ソフトを再起動します。
1. PostgreSQLを選択した上で、構築したデータベースの接続設定を行います(ここまで初回のみ)。

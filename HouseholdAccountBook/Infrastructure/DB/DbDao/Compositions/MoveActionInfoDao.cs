...
SELECT A.book_id, A.action_id, A.item_id, A.act_time, A.asset_id,
       (A.act_value / POWER(10, AA.scale)) * AA.base_rate / BA.base_rate AS main_act_value, BA.asset_id AS act_asset_id, -- 表示用の金額
       (A.act_value / POWER(10, AA.scale)) AS org_main_act_value, AA.asset_id AS org_act_asset_id, -- 編集用の金額
       A.remark, I.item_kind, I.move_flg
FROM hst_action A
...
INNER JOIN mst_asset AA ON AA.asset_id = COALESCE(A.asset_id, I.asset_id, B.asset_id, @DefaultAssetId) AND AA.del_flg = 0 -- 編集するアセット(帳簿項目に紐づくアセット)

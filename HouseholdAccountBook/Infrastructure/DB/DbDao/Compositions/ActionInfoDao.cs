...
new { GroupId = groupId, DefaultAssetId = defaultAssetId });
        ...
        /// <returns>取得したレコードリスト</returns>
        public async Task<IEnumerable<ActionInfoDto>> FindAllWithinPeriodAsync(int defaultAssetId, DateOnly startDate, DateOnly finishDate)
        {
...
new { DefaultAssetId = defaultAssetId, StartDate = startDate, FinishDate = finishDate.AddDays(1) });
        ...
        /// <param name="finishDate">終了日付(該当の日を含む)</param>
        /// <returns>取得したレコードリスト</returns>
        public async Task<IEnumerable<ActionInfoDto>> FindByBookIdWithinPeriodAsync(int bookId, int defaultAssetId, DateOnly startDate, DateOnly finishDate)
        {
